...
# to move next. Moves are always exactly one house
# to the north (^), south (v), east (>), or west (<).
...
def calculate_houses
  moves = File.open('input.txt', 'r').read
  moves = moves.split("\n").first.chars
  ...
  moves.each.with_index(1) do |move, index|
    last_santa_house = santa_houses.last
    last_robo_house = robo_houses.last

    last_house = index.odd? ? last_santa_house : last_robo_house
    ...
    houses = index.odd? ? santa_houses : robo_houses
    houses << new_house
  end
